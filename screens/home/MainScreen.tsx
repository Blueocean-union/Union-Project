  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.bottomNav}>
        {[
          { icon: 'document-text-outline', text: '자료' },
          { icon: 'calendar-outline', text: '일정' },
          { icon: 'home', text: '홈', active: true },
          { icon: 'search-outline', text: '검색' },
          { icon: 'chatbubble-outline', text: '채팅' }
        ].map((item, index) => (
          <TouchableOpacity key={index} style={styles.navItem}>
            <Ionicons name={item.icon as any} size={24} color={item.active ? "#3B4B87" : "#9CA3AF"} />
            <Text style={[styles.navText, item.active && styles.activeNavText]}>{item.text}</Text>
          </TouchableOpacity>
        ))}
      </View>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  bottomNav: { flexDirection: 'row', justifyContent: 'space-around', paddingVertical: 12, paddingHorizontal: 20, backgroundColor: '#FFFFFF', borderTopWidth: 1, borderTopColor: '#E5E7EB', position: 'absolute', bottom: 0, left: 0, right: 0 },
